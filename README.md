...
| Process and PTY | Official and A3S Python sync/async and TypeScript clients pass foreground and background commands, list, stdin send/close, wait, PTY create/resize/input/wait, and ordered output on real Sandboxes; the host broker also has bounded, fragmented, ordered JSON client-streaming input coverage | Additional signals, binary framing, reconnect, cancellation/backpressure stress, and adversarial concurrent-stream coverage |
