...
| MicroVM lifecycle | Guest stop is skipped when the workload already exited; Unix cold boot fail-closes without an exec heartbeat; crash-detection grace is 80ms. Transport retries cover ambiguous ACK/stream loss on keyed exec, read-only filesystem ops, keyed mutating filesystem ops (`MakeDir`/`Move`/`Remove` with `request_id`), keyed file uploads, and file downloads. |
...
> pause/resume, snapshot, restart, cleanup, and Native Live v4 retained stream
> + filesystem continuity across owner SIGKILL. Explicit `off` keeps the
