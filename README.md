...
| envd | Authenticated health and production runtime-envd coverage; Broker-mode MicroVMs also expose generation-fenced, binary-safe `GET /files` and raw or multipart `POST /files` through the guest session transport | Broker transfers are capped at 11 MiB per file; compression, ranges, metadata/xattrs, signed URLs, Filesystem Protobuf procedures, and production edge-case validation remain gates |
...
| Filesystem | The same six clients pass the production runtime-envd subset; the host broker now has bounded binary download, octet-stream upload, and multi-file multipart coverage with user-relative POSIX paths and generation fencing | Broker-side Protobuf metadata/directory operations, watches, streaming large files, ownership edge cases, and negative-path breadth |
