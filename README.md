...
> **Sandbox (Native Linux) claim surface:** Linux defaults new Sandbox records
> to the production OCI owner route (`SandboxViaOci`) without requiring
...
> gate (tip harness v7; CI-greened digests v7-scoped) for
...
> **Still open (out of Sandbox GA):** HVF MicroVM *production* composition
...
> Windows/WHPX omit-isolation → OCI DedicatedVm production cutover is
> tip-proven (gates 1–8 in
...
`--isolation sandbox` uses the production OCI owner route by default (see
below). This is not a preview API:
...
Sandbox production activation is the Linux default for `--isolation sandbox`
(not the default omit-isolation / MicroVM path). Install the pinned `a3s-oci`
and `a3s-oci-agent` pair. Artifact overrides are optional when the packaged
binaries are on the discovery path:
...
`a3s.box.linux-native-live-session.v7`, CI-greened digests v7-scoped) proves
**retained** streaming exec handles and filesystem continuity
...
continuity or Sandbox guest `filesystem_replay`. **CI-greened** Native Live v7
on PR #358 / run `34805883757` (report `box_commit_sha` `76dc560e…`; OCI
`931def0b…`): report SHA-256
