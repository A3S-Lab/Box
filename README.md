...
| Windows | Native x86_64 WHPX/libkrun MicroVM execution | Implemented and real-host validated; stable releases include a Windows x86_64 archive with the matching runtime DLLs and guest init. Foreground and detached workloads, long arguments staged outside the guest kernel command line, live split logs, exit codes, TCP publishing, bind mounts, named-volume persistence, stats, stopped-box commit, and stopped-box filesystem snapshot flows pass. Running-box commit has no guest archive channel. The reliable path is currently limited to one vCPU; container health checks, bridge networking, interactive execution, TEE, snapshot-fork, and CRI remain unsupported. |
...
| Windows | x86_64, Windows Hypervisor Platform, Developer Mode (or `SeCreateSymbolicLinkPrivilege`), and matching `krun.dll`/`libkrunfw.dll` assets | Install the stable Windows x86_64 release archive or build from source. Currently one vCPU, with the command boundaries documented below |
