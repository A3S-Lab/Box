...
- [ ] Keep image distribution, builds, named volumes, snapshots, and commits in
  ...
  **Partial:** Native Linux SandboxViaOci prepare now binds Box-owned named and
  ...
  closed like managed cleanup; SDK product `remove_box` / `prune_boxes` /
  `stop_box` route managed executions through the execution manager (no
...
- [ ] Keep network objects, IPAM, DNS, aliases, and publication policy in Box;
  ...
  **Partial:** Native Linux SandboxViaOci prepare classifies prepared
  ...
  keep-authority DNAT. Netproxy admits static UDP (`host:guest/udp`) with
  per-client NAT associations; Runtime Service endpoints also admit UDP with
  generation-fenced host listeners and guest associations (MicroVM
  `FRAME_OPEN_UDP` / Sandbox netns UDP). Health probes remain TCP-only.
  CRI admits static UDP into the same `host:guest/udp` contract.
  ...
  `base_v2` loopback-only. Netproxy Bridge (macOS) and Linux passt_bridge
  ...
  open. Explicit `--dns` must be IPv4 (IPv6 and garbage fail closed). Does
...
- [ ] Add quiesce/resume integration for consistent stopped and online product
  ...
  boot instead of soft success; CLI boot/start/restart/run also fail closed on
  baseline creation (no warn-and-continue); boot fails closed when
...
- [ ] Run Compose, restart policy, health monitoring, and warm-pool scheduling
  ...
  **Partial:** Compose `--isolation sandbox` up/down now uses
  ...
  invent-clean undo); CLI `prune`/`system-prune`/`image-prune`/`rmi`/`info`
  fail closed on host wipe, unused-image remove (including non–not-found
  `rmi --force` remove Err), unused-network reap, and inventory/image-store
  open (SDK / `rmi` / `network prune` / `ps` parity). Warm-pool, MicroVM
