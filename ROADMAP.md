...
- [ ] Support Windows bind mounts and named volumes without weakening Linux
  ...
  **Partial:** bind/named-volume prepare and managed VolumeStore paths refuse
  symlink/reparse host sources before following them. Host `:ro` write denial
  on virtio-fs and full Linux UID/GID storage on Windows binds remain open.
