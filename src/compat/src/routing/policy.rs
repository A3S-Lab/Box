...
mod tests {
    ...
    #[test]
    fn policy_rejects_zero_duplicates_and_excessive_ports() {
        assert_eq!(
            SandboxRoutePolicy::new([(ENVD_PORT, TokenScope::Envd), (0, TokenScope::Traffic),])
                .unwrap_err(),
            RoutePolicyError::InvalidPort(0)
        );
        assert_eq!(
            SandboxRoutePolicy::new([
                (ENVD_PORT, TokenScope::Envd),
                (8080, TokenScope::Traffic),
                (8080, TokenScope::Traffic),
            ])
            .unwrap_err(),
            RoutePolicyError::InvalidPort(8080)
        );

        let excessive = std::iter::once((ENVD_PORT, TokenScope::Envd))
            .chain((1..=64).map(|port| (port, TokenScope::Traffic)));
        assert_eq!(
            SandboxRoutePolicy::new(excessive).unwrap_err(),
            RoutePolicyError::TooManyPorts
        );
    }
}
