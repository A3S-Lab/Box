...
mod tests {
    ...
    #[tokio::test]
    async fn rejects_missing_duplicate_oversized_expired_and_unknown_routes() {
        let harness = Harness::new().await;
        let route = harness.route(ENVD_PORT);

        assert!(matches!(
            harness.service.resolve(&route, &HeaderMap::new()).await,
            Err(RouteLeaseError::MissingToken)
        ));

        let mut duplicate = token_headers(ENVD_ACCESS_TOKEN_HEADER, &harness.envd_secret);
        duplicate.append(
            ENVD_ACCESS_TOKEN_HEADER,
            HeaderValue::from_str(harness.envd_secret.expose_secret()).unwrap(),
        );
        assert!(matches!(
            harness.service.resolve(&route, &duplicate).await,
            Err(RouteLeaseError::InvalidToken)
        ));
        ...
        assert!(matches!(
            harness.service.resolve(&route, &oversized).await,
            Err(RouteLeaseError::InvalidToken)
        ));

        let unknown = ParsedSandboxRoute {
            sandbox_id: SandboxId::new("missing-route").unwrap(),
            port: NonZeroU16::new(ENVD_PORT).unwrap(),
            form: super::super::RouteForm::Direct,
        };
        assert!(matches!(
            harness
        ...
        let mut record = harness
            ...
            .unwrap();
        let expected = record.generation();
        record.replace_expiry(harness.now).unwrap();
        harness
            .repository
            .compare_and_swap(&harness.sandbox_id, expected, record)
            .await
