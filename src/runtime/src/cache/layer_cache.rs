...
pub(crate) fn copy_file_cow(src: &Path, dst: &Path) -> std::io::Result<()> {
    #[cfg(target_os = "linux")]
    {
        ...
        let reflinked = (|| -> std::io::Result<bool> {
            ...
            // `libc::Ioctl` is `c_ulong` on glibc and `c_int` on musl. Use
            // libc's target-specific constant instead of fixing the request to
            // one ABI's integer type.
...
mod tests {
    ...
    fn test_copy_dir_recursive_preserves_windows_symlink() {
        ...
        std::fs::create_dir_all(&src).unwrap();
        std::fs::write(src.join("target.txt"), "target").unwrap();
        if let Err(error) = std::os::windows::fs::symlink_file("target.txt", src.join("link.txt")) {
            // Windows requires Developer Mode or SeCreateSymbolicLinkPrivilege.
            // Keep exercising the copy when the host supports symlinks, while
            // allowing unprivileged Windows CI and developer shells to proceed.
            const ERROR_PRIVILEGE_NOT_HELD: i32 = 1314;
            if error.raw_os_error() == Some(ERROR_PRIVILEGE_NOT_HELD) {
                return;
            }
            panic!("failed to create Windows test symlink: {error}");
        }
