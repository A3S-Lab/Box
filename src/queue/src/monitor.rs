...
mod tests {
    ...
    use a3s_box_core::queue::{lane_ids, priorities, Command, Lane};
    ...
    async fn make_queue_with_lanes() -> Arc<CommandQueue> {
        ...
        for (id, priority, max) in lanes {
            ...
            queue.register_lane(Arc::new(Lane::new(id, config, priority))).await;
        }
    ...
    impl Command for TestCommand {
        async fn execute(&self) -> a3s_box_core::error::Result<serde_json::Value> {
