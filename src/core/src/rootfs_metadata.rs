...
/// Return the canonical mode for system files generated by the runtime.
///
...
/// an older manifest mode after that write would make the active resolver or
/// hostname configuration inaccessible to non-root image users.
pub fn runtime_managed_system_file_mode(path: &Path) -> Option<u32> {
    match path.to_str() {
...
mod tests {
    ...
    #[test]
    fn runtime_managed_system_files_have_canonical_modes() {
        for path in ["etc/hostname", "etc/hosts", "etc/resolv.conf"] {
            assert_eq!(
                runtime_managed_system_file_mode(Path::new(path)),
                Some(0o644)
            );
        }
        assert_eq!(
            runtime_managed_system_file_mode(Path::new("etc/passwd")),
            None
        );
    }
}
