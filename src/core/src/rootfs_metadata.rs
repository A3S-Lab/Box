...
use crate::guest_boot::GUEST_READY_PATH;
...
pub fn is_runtime_internal_rootfs_path(path: &Path) -> bool {
    ...
    let guest_ready_name = GUEST_READY_PATH.trim_start_matches('/');
    #[cfg(windows)]
    ...
    #[cfg(windows)]
    if name.eq_ignore_ascii_case(guest_ready_name) {
        return true;
    }
    #[cfg(not(windows))]
    if name == guest_ready_name {
        return true;
    }
    const INTERNAL: &[&str] = &[
...
pub fn runtime_managed_rootfs_mode(path: &Path) -> Option<u32> {
    match path.to_str() {
        ...
        Some(path) if path == GUEST_READY_PATH.trim_start_matches('/') => Some(0o600),
...
mod tests {
    ...
    fn runtime_managed_rootfs_files_have_canonical_modes() {
        ...
        assert_eq!(
            runtime_managed_rootfs_mode(Path::new(".a3s_guest_ready")),
            Some(0o600)
        );
