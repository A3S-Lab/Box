...
Python Code Interpreter execution plus context create/list/run/restart/remove.
...
compatibility. Templates, snapshots, signed files, filesystem-only pause,
historical metrics, MCP, additional signals, reconnect, cancellation,
backpressure, multi-file and large-file behavior, deeper Volume failure and
recovery cases, and other pinned edge cases remain outside the matrix, so
