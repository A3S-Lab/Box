...
# runs the retained-manager Live reopen harness:
#   create+start MicroVM → streaming start_process → Host SIGKILL →
#   Unavailable → replacement Host (SESSION_OWNER=1) → reconcile Ready →
#   same-handle stdin/signal/Exit → keyed exec / inventory / stats / kill
#
# Observation-only. Does not close B2 and does not overload the stopped-only
# linux-kvm-oci-qualification schema. Leave ROADMAP Utility-VM/KVM Live
# unchecked until greened on /dev/kvm.
...
Forces A3S_OCI_KVM_SESSION_OWNER=1 on first Host spawn (replacement is also
spawned with SESSION_OWNER=1 from the example).
EOF
}
...
# Live path: durable session-owner must outlive Host Service SIGKILL.
...
echo "  session-owner=A3S_OCI_KVM_SESSION_OWNER=1"
echo "  image=${IMAGE}"
echo "  box-sha=${BOX_SHA}"
echo "  oci-sha=${OCI_SHA}"
echo "  report=${REPORT}"
echo "  note: harness only; ROADMAP Utility-VM/KVM Live stays open until greened"
