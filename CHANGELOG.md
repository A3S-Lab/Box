...
- Windows MicroVM virtio-fs writable binds: honor FUSE `ATOMIC_O_TRUNC` on
  ...
  `test_virtiofs_windows_open_o_trunc_shrinks_existing_file`. Locked into
  ignored `core_smoke`
  `real_core_writable_bind_mount_truncates_on_overwrite` and the WIN-01 soak
  matrix (13 tests with virtiofs tar). Does **not** claim Enterprise GA or
  close full Linux UID/GID storage on Windows binds.
...
- Fix Windows WHPX soak (`WIN-01`) matrix: drop Linux-only `core_smoke` names
  ...
  (`result=pass`, nine tests). Does **not** claim Enterprise GA or close
  longitudinal multi-iteration trend verification.
...
- Windows Box-owned WHPX Host spawn passes `--system-image-manifest` required
  ...
  artifact `system-image\system-image.json`. Packaged Host discovery
  (`oci_whpx_packaged`) resolves `a3s-oci.exe`, shim, `bootstrap-vm-rootfs`,
  and `system-image.json` for opt-in `microvm|all` without endpoint (binder
  gates 1–2). Does **not** flip absent-env gate 5 or claim Enterprise GA
  (#650).
...
- R17 MicroVM security evidence reads the redacted virtio-fs secret-binding
  ...
  matcher accepts the product's `tee/sev-snp-simulated` and
  `tee/sev-snp-hardware` suffix. Secret values stay in the consumed boot
  bundle, not in that durable file. Does **not** close B2/B4 or claim
  Enterprise GA. KVM/SEV-SNP profile runs remain host evidence, not this
  change by itself.
...
- Operator `deny` and the default untrusted profile on passt_bridge are
  ...
  gets a local TCP/53 reply or a NetworkStore answer. Peer frames are still
  delivered. Does **not** close B3 or claim Enterprise GA.
...
- passt is started with `--no-map-gw`, so the guest gateway is not rewritten
  ...
  destinations stay allowed. Does **not** close B3 or claim Enterprise GA.
...
- Runtime Service endpoints admit UDP: advertise `ServiceUdp`, bind
  ...
  probes stay TCP-only. Does **not** close ROADMAP B3/B2 or unlock CNI/DNS.
- Netproxy admits static UDP published ports (`host:guest/udp`) with per-client
  ...
  admission. Unresolved `host_port=0` still fails closed. Does **not** close
  ROADMAP B3/B2 or unlock CNI/DNS.
...
- Runtime `NetworkMode::Outbound` is advertised and mapped to MicroVM/libkrun
  ...
  `UnsupportedCapabilities`, matching host-sandbox design (no workload egress).
  Does **not** implement Sandbox L3 egress, complete Cloud sole-Runtime
  cutover, Bollard removal, or clean-host re-cert.
- Refresh bundled `vendor/libkrun-source.tar` (and digests) for vendored
  libkrun `f7c6918` so Windows WHPX packaging checks match the submodule pin
  after #465. Does **not** change the packaged Windows runtime DLLs.
...
- Abandoned managed `Creating` / `Starting` claims (client death mid-`run`)
  ...
  so operators do not race an in-flight start. Closes #372. Manager inspect
  under the execution lifecycle lock retires durable `Starting` when backend
...
- CRI `CreateContainer` / `PortForward` / `UpdatePodSandboxResources` refuse
  ...
  or flip B2.
- CRI `ExecSync` / `Exec` / `Attach` / `UpdateContainerResources` refuse
...
- CRI `StartContainer` refuses inventing "already running" from durable
  ...
  or flip B2.
- CRI `ContainerStats` / `ListContainerStats` / `StreamContainerStats` refuse
  inventing Running resource usage from durable store alone — re-prove
  sandbox VM health and demote before reporting (#442 / #434/#436). Does
  **not** invent Live digests or flip B2.
- CRI `PodSandboxStats` / `ListPodSandboxStats` / `StreamPodSandboxStats`
...
- CRI `UpdatePodSandboxResources` refuses inventing Ok when `linux` or
  ...
  **not** invent Live digests or flip B2.
- CRI `UpdateRuntimeConfig` refuses inventing Ok when `NetworkConfig.pod_cidr`
...
- Publish CI-greened Native Live
  ...
  “v4-scoped tip greened digests” claims. Does **not** flip B2 or invent
  digests.
...
- Live-session observation harness bumps to Native
  ...
  artifact labels track tip schema v7. Does **not** flip B2.
...
- Language SDK tests lock MakeDir / Move / Remove success `request_id`
  ...
  `scripts/verify-fs-mutate-request-id-honesty.py` fail-closes if Rust /
  bridge / language SDK surfaces or those tests regress to ok-only/void.
  Does **not** flip B2 or invent Live digests.
...
- MicroVM `LocalExecutionManager::filesystem` now shares the guest filesystem
  ...
  from #344 reach the production session path. Does **not** flip B2 or claim
  Sandbox Live FS recovery closed.
...
- Fresh `SandboxViaOci` construction reaps orphaned supervised sessions
  ...
  smoke-script SIGKILL workaround. Retained-manager Live reopen keeps the
  default (no reap) so Host SIGKILL can reopen an alive session. SDK sandbox
  smoke asserts the supervisor survives Host death until the fresh Box ensure
  path tears it down. Does **not** flip `b2_process_session_recovery_closed`,
  MicroVM cutover, or WHPX/KVM MicroVM production claims.
...
- Box-owned native Linux Host spawn now forces
  ...
  requires `sessionSupervisor` in `native-linux-recovery.v6`. External
  operator-launched Hosts that omit the env remain Host-bound. Does **not**
  flip `b2_process_session_recovery_closed`, MicroVM cutover, or WHPX/KVM
  MicroVM production claims.
