...
| Typed box configuration | image, isolation, CPU/memory/lifetime, environment, workdir/user, mounts, tmpfs, network, ports, DNS/hosts, read-only root, persistence, cleanup, and snapshot restore | Implemented in Rust/Python/TypeScript options and fluent builders. The real macOS/HVF MicroVM builder gate passes; the no-KVM Ubuntu gate uses A3S OCI Runtime, with certified crun retained as a separate rollback differential, while Linux/KVM remains host-gated. |
