...
| `diff`, `export`, stopped-box `commit`, and stopped-box filesystem snapshots | Validated through clean-stop metadata capture, committed-image re-run, snapshot restore/restart, and guest-visible restored content (`exec`). Source-box stopped `export` covers the Windows metadata contract; stopped re-export of a snapshot-restored box after a second boot generation remains open when the guest does not rewrite terminal metadata. Running-box host-path capture remains unavailable because WHPX has no post-boot guest archive channel. |
