...
| `scripts/windows-whpx-soak.ps1` | Repeats the Windows-supported real tests (lifecycle, bind-mount `:ro`, virtiofs tar, volume-backed init, named volume, commit, snapshot, …) and rejects residual A3S Box processes | Uses a Windows-specific summary and has no shared cross-capability verifier or long-term handle/RSS slope gate. Pass `-SkipVirtiofsStress` to omit the longer virtiofs tar case |
...
| A3S Runtime Provider host | `G2` after provider/runtime changes, `R24` before provider promotion | All advertised R17 profiles on the certified Sandbox and Linux/KVM MicroVM backends |
...
| `PRO-01` | A3S Runtime provider | Repeat every advertised R17 Base, Recovery, Networking, Mounts, Health, Resources, Logs, Exec, Security, Outputs, and capability-triggered Evidence profile with driver restart between cycles; replay and rotate generation-fenced TCP Service endpoints under connection pressure; exercise distinct readiness/liveness paths, independent thresholds and transitions, liveness-driven recovery, cooperative graceful stop, deadline force-stop, probe timeouts/start periods, and Service exit during a probe | Advertised profile set stays constant; endpoint sockets remain exact and unique; stale generations close; readiness and liveness remain independent; an unhealthy generation restarts at most once per operation; the declared grace is neither truncated nor exceeded; health output stays undisclosed; attestation stays bound to the exact Runtime and Box generation; terminal lifecycle wins probe races; inventory before/after matches; and idempotent operations do not duplicate resources | Partial: one-shot destructive conformance is wired for Sandbox and Linux/KVM MicroVM, including the capability-triggered Service lifecycle cases; simulated SEV-SNP wiring is covered but is not hardware evidence; current-commit real-host results and the repeated `G2` soak remain required |
...
| `BLD-01` | Dockerfile builds | Repeated native builds through Linux chroot and warm-pool `RUN`, multi-stage targets, cache hit/miss/invalidation, concurrent shared/locked caches, failed `RUN`, save/load/push | Output filesystem and OCI metadata match expectations; failed builds publish no partial cache/layer; cache growth and latency remain bounded | Partial |
...
| `TEE-01` | TEE | Repeated real SNP attestation, policy verification, RA-TLS, attestation-before-execution for Tasks and Services, seal/unseal, rollback rejection, secret injection/rotation, restart, and negative evidence | Every report verifies against the recorded policy; no confidential main starts before its generation-bound artifact is published; stale/tampered evidence fails closed; secrets do not appear in logs/evidence and remain unavailable outside the intended guest | Partial: Runtime-bound simulated RA-TLS regression and an opt-in hardware gate exist; executed hardware soak evidence remains required, and simulation never satisfies this row |
...
| `WIN-01` | Windows | Repeat every supported WHPX core-smoke test (lifecycle, create/start override, foreground exit/logs, long argv, cp/top/stats, published ports, named volumes, commit ownership, filesystem snapshots) | Every listed test passes within its timeout with runtime samples; no A3S CLI/shim/forwarder process remains; handle/RSS/disk trends remain within recorded bounds | Tip-proven one-iteration (`93c08510…` on `c8c2c1dc`) and **three-iteration longitudinal** summary SHA-256 `0ff23f04a470e6de12f616b3a9d29ea4aa5be10d7a90fa4b152351e313bae52e` (Box `1c741962…`; `result=pass`, `verification=pass`, 3×9 tests). Bind/virtiofs tar stress stay Linux-only. Longer duration/`R24` trend gates remain open for Enterprise promotion |
...
| Build or warm pool | `R0 BLD-01` and `POL-01` | `G2`; `R24` for native build-cache, lease, or snapshot-fork changes |
