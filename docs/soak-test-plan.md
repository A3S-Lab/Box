...
| `WIN-01` | Windows | Repeat every supported WHPX core-smoke test (lifecycle, create/start override, foreground exit/logs, long argv, cp/top/stats, published ports, named volumes, commit ownership, filesystem snapshots) | Every listed test passes within its timeout with runtime samples; no A3S CLI/shim/forwarder process remains; handle/RSS/disk trends remain within recorded bounds | Existing core loop on tip (summary digest `93c08510…`, Box `c8c2c1dc…`); longitudinal multi-iteration trend verification remains partial. Bind/virtiofs tar stress stay Linux-only |
