...
| `PRO-01` | A3S Runtime provider | Repeat every advertised R17 Base, Recovery, Networking, Mounts, Resources, Logs, Exec, and Security profile with driver restart between cycles; replay and rotate generation-fenced TCP Service endpoints under connection pressure | Advertised profile set stays constant; endpoint sockets remain exact and unique, stale generations close, inventory before/after matches, and idempotent operations do not duplicate resources | Planned: destructive real conformance is one-shot today |
