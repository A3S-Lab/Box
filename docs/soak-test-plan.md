...
| `WIN-01` | Windows | Repeat every supported WHPX core-smoke test (lifecycle, create/start override, foreground exit/logs, long argv, cp/top/stats, published ports, BindFlt `:ro` binds, writable-bind `O_TRUNC` overwrite, virtiofs tar, named volumes, commit ownership, filesystem snapshots) | Every listed test passes within its timeout with runtime samples; no A3S CLI/shim/forwarder process remains; handle/RSS/disk trends remain within recorded bounds | Tip-proven one-iteration through R24 digests on Box tip (see CHANGELOG / windows-whpx.md); writable-bind truncate locked as `real_core_writable_bind_mount_truncates_on_overwrite`. Enterprise GA / B3–B6 remain open |
