...
| `scripts/windows-whpx-soak.ps1` | Repeats the Windows-supported real tests (lifecycle, bind-mount `:ro`, volume-backed init, named volume, commit, snapshot, …) and rejects residual A3S Box processes | Uses a Windows-specific summary and has no shared cross-capability verifier or long-term handle/RSS slope gate. Virtiofs tar stress stays Linux-only in `core_smoke` |
