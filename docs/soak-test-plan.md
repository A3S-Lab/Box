...
| `scripts/windows-whpx-soak.ps1` | Repeats the nine Windows-supported real tests and rejects residual A3S Box processes | Uses a Windows-specific summary and has no shared cross-capability verifier or long-term handle/RSS slope gate. Bind mounts, volume-backed init, and virtiofs tar stress stay Linux-only in `core_smoke` |
