...
- Native x86_64 Windows validation on July 20–21, 2026 covered Alpine 3.20
  ...
  `inspect`, unchanged success/non-zero workload exit codes, sequential
  published-port connections, drive-letter directory and read-only single-file
  binds, named-volume restart persistence, `stats --no-stream`, and the
  `diff`/`export`/`commit`/filesystem-snapshot lifecycle. All 113 Linux
...
- Native WHPX bridge networking is not implemented. Standard Compose services
  ...
  the current Windows support boundary along with interactive PTY, `attach`,
  post-boot `exec`, memory snapshot-fork, TEE, and CRI.
...
5. Add explicit Windows diagnostics for bridge-network/Compose requests and
   continue closing the documented interactive PTY, `attach`, and post-boot
   `exec` support gaps.
