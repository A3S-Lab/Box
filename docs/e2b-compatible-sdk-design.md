...
2. **Complete:** add the owner-scoped HTTP lifecycle router and run the
   ...
   manager.
3. **In progress:** SQLite WAL migrations, strict compare-and-swap repository
   operations, restart recovery, and corruption tests are complete. Expiry
   reaping and startup reconciliation remain in the next immediate batch.
