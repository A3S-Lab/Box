...
An image defines the read-only starting point. Continue with
[Storage and snapshots](./storage-snapshots/) to choose a writable layer,
volume, or snapshot for runtime data. Open the [SDK overview](/sdk/) when the
image build belongs in application code.
